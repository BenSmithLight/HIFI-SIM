function HIFI_from_image_stack(filename, image_frame)
    %% 读取TIF文件
    % clear, clc, close all
    ...
    % 提取图片的名字
    image_name = regexp(filename, '[^/]+$', 'match', 'once');
    programroot = strrep(rootpath, 'rawdata', 'processed_data');
    outputroot = [programroot, '_HIFI/'];
    tifroot = [programroot, '_merge/'];
    outputname = [outputroot, image_name(1:end - 4), '_', num2str(image_frame), '_HIFI.tif'];
    tifname = [tifroot, image_name(1:end - 4), '_', num2str(image_frame), '_merge.tif'];
    ...
    if ~exist(tifroot, 'dir')
        mkdir(tifroot);
    end

    data = imread(filename, image_frame);
    
    for i = image_frame + 1:image_frame + 8
        data = cat(3, data, imread(filename, i));
    end
