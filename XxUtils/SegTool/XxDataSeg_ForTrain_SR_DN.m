% ------------------------------------------------------------------------
% 该函数用于同时为SR和DN网络训练准备数据
% ------------------------------------------------------------------------

function [data_seg, gt_seg, gt_seg2] = XxDataSeg_ForTrain_SR_DN(data, gt, gt2, num_seg, Nx, Ny, RotFlag)

    % 该函数用于同时为SR和DN网络训练准备数据
    % 函数基于DN网络，增加部分（编号2）为SR网络数据
    ...
    % SR部分
    gt_cal_mask2 = imresize(gt2, 1 / sr_ratio, 'bicubic');
    mask2 = XxCalMask(gt_cal_mask2, ksize, thresh_mask);
    % DN部分
    ...
    for i = 1:num_seg
        ...
        if RotFlag >= 1 % if random rotate 如果随机旋转
            ...
            % SR部分
            patch_gt2 = imrotate(gt2(sr_ratio * x1 - 1:sr_ratio * x2, sr_ratio * y1 - 1:sr_ratio * y2, 1), degree, 'bilinear', 'crop'); % 384*384 使用了sr_ratio来调整区域的大小
            tx_gt2 = round(new_r * sr_ratio / 2) - round(Nx * sr_ratio / 2); % gt_patch的起始坐标
            ty_gt2 = round(new_l * sr_ratio / 2) - round(Ny * sr_ratio / 2);
            patch_gt2 = patch_gt2(tx_gt2 + 1:tx_gt2 + sr_ratio * Nx, ty_gt2 + 1:ty_gt2 + sr_ratio * Ny); % 256*256
            sum_patch_gt2 = sum(patch_gt2(:));
            active_range_gt2 = double(prctile(patch_gt2(:), 99.9)) / double(prctile(patch_gt2(:), 0.1) + 1); % 对比度评估
            % DN部分
        ...
        while active_range_gt < thresh_ar_gt || sum_patch_gt < thresh_sum_gt || active_range < thresh_ar
            ...
            if RotFlag >= 1 % if random rotate
                ...
                % SR部分
                patch_gt2 = imrotate(gt2(sr_ratio * x1 - 1:sr_ratio * x2, sr_ratio * y1 - 1:sr_ratio * y2, 1), degree, 'bilinear', 'crop'); % 384*384
                tx_gt2 = round(new_r * sr_ratio / 2) - round(Nx * sr_ratio / 2); % gt_patch的起始坐标
                ty_gt2 = round(new_l * sr_ratio / 2) - round(Ny * sr_ratio / 2);
                patch_gt2 = patch_gt2(tx_gt2 + 1:tx_gt2 + sr_ratio * Nx, ty_gt2 + 1:ty_gt2 + sr_ratio * Ny); % 256*256
                sum_patch_gt2 = sum(patch_gt2(:));
        ...
        if RotFlag >= 1
            ...
            % SR部分
            tgt2 = imrotate(gt2(sr_ratio * x1 - 1:sr_ratio * x2, sr_ratio * y1 - 1:sr_ratio * y2, :), degree, 'bilinear', 'crop');
            % DN部分
            tgt = imrotate(gt(x1:x2, y1:y2, :), degree, 'bilinear', 'crop'); % 192*192*9

            if isempty(data_seg)
                data_seg = tdata(tx + 1:tx + Nx, ty + 1:ty + Ny, :); % 128*128*9
                data_seg2 = tdata(tx + 1:tx + Nx, ty + 1:ty + Ny, :); % 128*128*9
            else
                h = size(tdata, 3);
                data_seg(:, :, end + 1:end + h) = tdata(tx + 1:tx + Nx, ty + 1:ty + Ny, :);
                data_seg2(:, :, end + 1:end + h) = tdata(tx + 1:tx + Nx, ty + 1:ty + Ny, :);
            end

            if isempty(gt_seg)
                % SR部分
                gt_seg2 = tgt2(tx_gt2 + 1:tx_gt2 + sr_ratio * Nx, ty_gt2 + 1:ty_gt2 + sr_ratio * Ny, :); % 256*256
                % DN部分
                gt_seg = tgt(tx_gt + 1:tx_gt + Nx, ty_gt + 1:ty_gt + Ny, :); % 128*128*9
            else
                h_gt = size(tgt, 3);
                % SR部分
                gt_seg2(:, :, end + 1:end + h_gt) = tgt2(tx_gt2 + 1:tx_gt2 + sr_ratio * Nx, ty_gt2 + 1:ty_gt2 + sr_ratio * Ny, :);
                % DN部分
