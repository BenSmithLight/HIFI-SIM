function [data_seg, gt_seg, gt_ori_seg] = XxDataSeg_ForTest(data, gt, gt_ori, num_seg, Nx, Ny, RotFlag)
    ...
    %    GT_ORI_SEG  -- image patches of SIM SR data (ground truth with multiple frames)
    ...
    for i = 1:num_seg
        ...
        if RotFlag >= 1
            ...
            tgt_ori = imrotate(gt_ori(x1:x2, y1:y2, :), degree, 'bilinear', 'crop');

            if isempty(data_seg)
                data_seg = tdata(tx + 1:tx + Nx, ty + 1:ty + Ny, :);
                gt_ori_seg = tgt_ori(tx + 1:tx + Nx, ty + 1:ty + Ny, :);
            else
                h = size(tdata, 3);
                data_seg(:, :, end + 1:end + h) = tdata(tx + 1:tx + Nx, ty + 1:ty + Ny, :);
                gt_ori_seg(:, :, end + 1:end + h) = tgt_ori(tx + 1:tx + Nx, ty + 1:ty + Ny, :);
            end
