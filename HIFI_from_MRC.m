function HIFI_from_MRC(filename)
    %% 读取MRC文件并保存为tif格式
    % clear, clc, close all
    ...
    % 文件参数
    % filename = '../../Picture/Pre_process/Microtubules/RawSIMData_level_09.mrc';
    % tifname = [filename(1:end - 4), '.tif'];
    outputname = strrep(filename, 'Pre', 'After');
    tifname = [outputname(1:end - 4), '.tif'];
    outputname = [outputname(1:end - 4), '_HiFi.tif'];

    % 如果没有outputname所在的文件夹，则创建文件夹
    if ~exist(fileparts(outputname), 'dir')
        mkdir(fileparts(outputname));
    end
    ...
    param.micronsPerPixel = 62.6;  % TODO：关键参数
    param.NA = 1.35;  % TODO：关键参数 CCPs,F_actin:1.41  ER,MTs:1.35
    param.lambda = 488;  % TODO：关键参数
    param.attStrength = 0.9;
    param.a = 1; %  阻尼因子“damping” factor：β
    w1 = 1.2; % Initial optimization Wiener constant：[0.9-2.5]
    ApoFWHM = 99; % 如果设置为99，则使用默认；否则使用设定值
    WF_flag = 0;
    SR_flag = 1;
